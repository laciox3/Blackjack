...
class Kasa:
    def __init__(self, ilosc_poczatkowa):
        self.saldo = ilosc_poczatkowa

    def czy_wygrana(self, pkt_gracza, pkt_krupiera):
        if pkt_gracza > 21:
            return False
        if pkt_krupiera > 21:
            return True
    ...
    def zaktualizuj_saldo(self, stawka, pkt_gracza, pkt_krupiera):
        if pkt_gracza <= 21 and pkt_gracza == pkt_krupiera:
            print(f"REMIS! Stawka {stawka} wraca do Ciebie.")
        elif self.czy_wygrana(pkt_gracza, pkt_krupiera):
            print(f"WYGRANA! Wygrywasz {stawka} PLN.")
            self.saldo += stawka
        else:
...
class HistoriaRuchow:
    ...
    def zapisz_ruch(self):
        self.stos_ruchow.append("DOBRANIE")

    def cofnij(self, gra):
        if len(self.stos_ruchow) > 0:
            ostatni_ruch = self.stos_ruchow.pop()
